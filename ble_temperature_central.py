...
class BLETemperatureCentral:
    ...
    def _irq(self, event, data):
        ...
        if event == _IRQ_SCAN_RESULT:
            ...
            print(f"{data=}")
            print(f"{decode_name(adv_data)}")
            if adv_type in (_ADV_IND, _ADV_DIRECT_IND) and _ENV_SENSE_UUID in decode_services(
                adv_data
            ):
                if decode_name(adv_data) == "basman":
                    print("connecting to mpy-temp")
    ...
    def scan(self, callback=None):
        ...
        #scan throws an interrupt. This interrupt is handled by _irq
        #depending on the interrupt event _irq will do something.
    ...
    def _update_value(self, data):
        # Data is int16 in degrees Celsius with a resolution of 0.01 degrees Celsius.
        # No idea why he is using a struct.... doesn't make sense to me.
        # Maybe  because a struct is serialable?
        print(f"{str(data,'utf8')}")
